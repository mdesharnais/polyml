...
(*    fun debugFunctionEntryCode(name, argCode, argType, location, {debugEnv, mkAddr, level, lex, ...}) =
    ...
    |   codeGenerate(Labelled {base, recList, expType=ref (expType as LabelledType {recList = expTypeRecList, ...}), ...}, context as { level, mkAddr, debugEnv, lex, ...}) =
        let
            ...
            val baseDec =
                (case base of
                    NONE => []
                |   SOME {exp, ...} => [multipleUses (codegen (exp, context), fn () => mkAddr 1, level)])

            val updateDecs = List.map (fn {name, valOrPat, ...} =>
                (name, multipleUses (codegen (valOrPat, context), fn () => mkAddr 1, level))) recList

            val (selDecs, tupleElems) = List.foldr (fn ({name as fieldName, typeof as fieldType}, (selDecs, tupleElems)) =>
                (case List.find (fn (name, _) => fieldName = name) updateDecs of
                    ...
                    let
                        val SOME {exp = baseExp, expType = ref baseExpType, ...} = base
                        val selector = Selector {
                            name = fieldName,
                            labType = baseExpType,
                            typeof = mkFunctionType (baseExpType, fieldType),
                ...
                |   SOME (_, dec) => (* The label is in the update list. *)
                    (selDecs, #load dec level :: tupleElems))) ([], []) expTypeRecList

            (* val () = LEX.warningMessage (lex, LEX.nullLocation,
                "length baseDec = " ^ Int.toString (length baseDec) ^ ", " ^
                "length updateDecs = " ^ Int.toString (length updateDecs) ^ ", " ^
